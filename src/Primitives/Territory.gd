...
enum territory_type {normal, resource, destroyed}
...
export(territory_type) var type = territory_type.normal setget set_type
...
func get_polygon_color():
	if type == territory_type.normal:
		return color
	elif type == territory_type.resource:
		return color.lightened(0.3)
	elif type == territory_type.destroyed:
